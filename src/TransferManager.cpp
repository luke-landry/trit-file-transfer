...
void TransferManager::send_chunks(asio::ip::tcp::socket& socket, BoundedThreadSafeQueue<Chunk>& in_queue, std::atomic<bool>& chunk_processing_done){
    while(true){
...
void TransferManager::receive_chunks(asio::ip::tcp::socket& socket, BoundedThreadSafeQueue<Chunk>& out_queue, std::atomic<bool>& chunk_reception_done, uint32_t num_chunks){
