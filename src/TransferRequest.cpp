...
TransferRequest TransferRequest::from_file_paths(const std::unordered_set<std::filesystem::path>& file_paths){
    ...
    // Only store the relative generic paths from staging root directory because absolute paths should not be sent
    std::vector<FileInfo> file_infos;
    for(const auto& file_path : file_paths){
        auto relative_generic_path = std::filesystem::relative(file_path, std::filesystem::current_path()).generic_string();
    ...
    // Otherwise, use the max chunk size, and the transfer will be divided into multiple chunks
    uint32_t uncompressed_chunk_size = std::min(transfer_size, static_cast<uint64_t>(utils::MAX_UNCOMPRESSED_CHUNK_SIZE));

    // Last chunk contains remaining data, which is variable
    uint32_t uncompressed_last_chunk_size = transfer_size % uncompressed_chunk_size;

    // Yields number of chunks, and results in extra chunk if total size not perfectly divisible
    uint32_t num_chunks = (transfer_size + (uncompressed_chunk_size - 1)) / uncompressed_chunk_size;

    return TransferRequest(
        num_files, 
        transfer_size, 
        uncompressed_chunk_size, 
        uncompressed_last_chunk_size, 
        num_chunks,
        file_infos
    );
}
...
/*
    This method deserializes the file transfer request based on the following format
    --------------------------
    ...
    fileN size [8 bytes]
*/
TransferRequest TransferRequest::deserialize(const std::vector<uint8_t>& buffer){
