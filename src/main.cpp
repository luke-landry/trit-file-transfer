...
void handle_send(const std::vector<std::string> &args) {
  ...
  if (args.size() != 2 && args.size() != 3) {
    std::cerr
        << "trit: 'send' requires an ip, port, and optionally a password.\n";
...
void handle_receive(const std::vector<std::string> &args) {
  ...
  /*
      Tried to pick a port number that was unreserved, unlikely to be used by
     other applications, and easy to type. Found an interesting website to
     search port information: https://www.speedguide.net/ports.php

      Was going to go with 50000, but this is used by many applications
      Then looked at 500001, but it turns out that Discord uses ports
     50001-50004 Tried 54321 but this is used by some software and unfortunately
     a bunch of malware Also tried 55555 but this one is associated with some
     malware as well Looked at 55000 but this is sometimes used by uTorrent and
     some other things as well Then looked at 50505 and 51515 but these are used
     by some applications Finally looked at 52525 and it had no documented
     existing users, so I picked this number
  */
  ...
  while (!utils::local_port_available(port)) {
    // Using randomly generated unreserved port for receiver if default is in
    // use
...
int main(int argc, char *argv[]) {
  ...
  const std::unordered_map<std::string, HandlerFunction> command_dispatch_map =
      {{"add", handle_add},        {"drop", handle_drop}, {"list", handle_list},
       {"clear", handle_clear},    {"help", handle_help}, {"send", handle_send},
