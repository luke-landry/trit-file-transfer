...
// Trick to get local IP address by creating a UDP socket and connecting to a
// remote endpoint (Google DNS)
...
template <>
std::string
input<std::string>(const std::string &prompt,
...
// Caller must resize out_value string to their desired length before calling
// this function
template <>
...
deserialize<std::string>(const std::vector<uint8_t>::const_iterator it,
  ...
  if (buffer_distance < string_size) {
...
std::string format_data_size(uint64_t size_in_bytes) {
  ...
  // First element is a dummy element to align powers of 1000 indices with
  // corresponding units
