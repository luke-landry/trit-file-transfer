...
// Using namespace instead of class to group utility functions since they are
// stateless and no data is shared between them
namespace utils {
...
relative_to_cwd(const std::unordered_set<std::filesystem::path> &paths);
...
/*
Prompts for and validates user input and checks it against a vector of expected
values. If a vector of expected values is not provided, then any valid input is
allowed.
*/
template <typename T>
T input(const std::string &prompt = "",
  ...
  while (!input_valid) {
    ...
    if ((!expected_values.empty()) &&
        (std::find(expected_values.begin(), expected_values.end(), input) ==
         expected_values.end())) {
...
// Specialization of input function template necessary for getting entire line
// of string
template <>
...
void serialize(const T &value, std::vector<uint8_t> &buffer) {
  ...
  static_assert(std::is_trivially_copyable_v<T>,
                "T must be trivially copyable to be serialized properly");
...
// assumes buffer byte order is the same as native byte order (should be little
// endian)
template <typename T>
std::vector<uint8_t>::const_iterator
deserialize(const std::vector<uint8_t>::const_iterator it,
            const std::vector<uint8_t>::const_iterator end, T &out_value) {

  static_assert(std::is_trivially_copyable_v<T>,
                "T must be trivially copyable to be deserialized properly");

  size_t T_size = sizeof(T);
  auto buffer_distance = end - it;

  if (buffer_distance < 0) {
    throw std::runtime_error("Invalid iterator range (end before it)");
  }
  if (buffer_distance < T_size) {
    throw std::runtime_error("Invalid buffer size");
  }

  // Vectors store elements contiguously, so its safe to use pointers to
  // dereferenced iterator values for memcpy
...
template <>
std::vector<uint8_t>::const_iterator
deserialize<std::string>(const std::vector<uint8_t>::const_iterator it,
