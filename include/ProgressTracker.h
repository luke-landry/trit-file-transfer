...
public:
  ProgressTracker(std::string name, T limit) : name_(name), limit_(limit) {
    static_assert(std::is_integral_v<T>,
                  "Template type of progress tracker must be integral");
    if (limit_ < 1) {
  ...
  void start(WorkerContext &ctx, const std::atomic<T> &value) {
    ...
    // For progress tracking, T is expected to start at 0 and increase
    // monotonically
    ...
    while (true) {
      ...
      if (current_value != last_value) {
        double progress_percentage =
            static_cast<double>(current_value) / limit_ * 100.0;
        const int bar_width = 30;
        constexpr char progress_fill_char = '#';
        constexpr char progress_empty_char = '-';
        int progress_fill_num_chars =
            static_cast<int>((progress_percentage / 100.0) * bar_width);
        int progress_empty_num_chars = bar_width - progress_fill_num_chars;

        std::string progress_bar = "[";
        progress_bar +=
            std::string(progress_fill_num_chars, progress_fill_char);
        progress_bar +=
            std::string(progress_empty_num_chars, progress_empty_char);
        progress_bar += "]";
